...
namespace DarkCanvas.Assets.Scripts.ProceduralTerrain
{
    public static class MeshGenerator
    {
        public static MeshData GenerateTerrainMesh(
            ...
            int levelOfDetail)
        {
            var threadSafeHeightCurve = new AnimationCurve(heightCurve.keys);
            ...
            var meshData = new MeshData(verticiesPerLine);
            ...
            for (var y = 0; y < borderedSize; y += meshSimplificationIncrement)
            {
                for (var x = 0; x < borderedSize; x += meshSimplificationIncrement)
                {
                    ...
                    var height = threadSafeHeightCurve.Evaluate(heightMap[x, y]) * heightMultiplier;
